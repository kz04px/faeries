...
pub fn listen(state: &mut Box<dyn UGI>) -> std::io::Result<()> {
    ...
    loop {
        ...
        while let Some(word) = stream.next() {
            match word {
                ...
                "position" => position::parse(&mut stream, |fen| {
                    state.position(fen);
                }),
                "moves" => moves::parse(&mut stream, |movestr| {
                    state.moves(movestr);
                }),
                "go" => go::parse(&mut stream, |settings| match settings.kind {
                    go::GoKind::Search => state.go(settings),
                    go::GoKind::Perft => state.perft(settings),
                    go::GoKind::FastPerft => todo!(),
                    go::GoKind::SplitPerft => state.split(settings),
                }),
                "perft" => {
