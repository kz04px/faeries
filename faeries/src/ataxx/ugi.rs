...
pub fn as_ugi(mv: &AtaxxMove) -> String {
    match mv {
        AtaxxMove::Pass => "0000".to_string(),
        AtaxxMove::Single(to) => format!("{}", to).to_string(),
        AtaxxMove::Double(from, to) => format!("{}{}", from, to).to_string(),
    }
}
...
impl UGI for AtaxxState {
    ...
    fn uginewgame(&mut self) {
        ...
        self.pos = AtaxxPosition::from_fen("startpos");
    }
