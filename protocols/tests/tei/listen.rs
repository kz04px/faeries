...
impl TEI for FakeState {
    ...
    fn teinewgame(&mut self, size: i32, halfkomi: i32) {
        self.received
            .push(format!("teinewgame size {} halfkomi {}", size, halfkomi));
    }
    ...
    fn position(&mut self, tps: &str) {
        self.received.push(format!("position tps {}", tps));
    }
    ...
    fn go(&mut self, _settings: &protocols::GoSettings) {
        self.received.push("go".to_string());
    }

    fn perft(&mut self, _settings: &protocols::GoSettings) {
        self.received.push("perft".to_string());
    }

    fn split(&mut self, _settings: &protocols::GoSettings) {
...
mod listen {
    ...
    fn test_inputs() {
        let tests = vec![
            ...
            (
                vec!["teinewgame size 3", "quit"],
                vec!["init", "teinewgame size 3 halfkomi 0", "shutdown"],
            ),
            (
                vec!["teinewgame size 3 halfkomi 4", "quit"],
                vec!["init", "teinewgame size 3 halfkomi 4", "shutdown"],
            ),
            (
                vec!["position startpos", "quit"],
                vec!["init", "position tps startpos", "shutdown"],
            ),
            ...
            (
                ...
                vec!["init", "isready", "position tps startpos", "shutdown"],
            ),
            (
            ...
            (
                vec!["isready", "position tps startpos", "quit"],
                vec!["init", "isready", "position tps startpos", "shutdown"],
            ),
            (
                vec!["isready", "position tps startpos moves", "quit"],
                vec!["init", "isready", "position tps startpos", "shutdown"],
            ),
            (
                vec!["isready", "position tps 1 2 3 4 5 6 moves", "quit"],
                vec!["init", "isready", "position tps 1 2 3 4 5 6", "shutdown"],
            ),
            (
                vec!["isready", "position tps 1 2 3 4 5 6 moves 7 8", "quit"],
                vec![
                    "init",
                    "isready",
                    "position tps 1 2 3 4 5 6",
                    "moves 7",
                    "moves 8",
                    "shutdown",
                ],
        ...
        for (inputs, outputs) in tests {
            ...
            let result: Result<(), std::io::Error> = tei::listen::listen(&mut state, get_input);
